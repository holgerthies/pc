...
ll cost(vector<pii>& pl, int start_pos){
	int left=start_pos;
	ll ans=0;
	for(auto p : pl){
		ans += abs(p.first-left);
		left += p.second;
	}
	return ans;
}

ll bin_search(vector<pii>& pl, int left, int right){
	if(right-left < 1){
		ll c1=cost(pl, left);
		ll c2=cost(pl, right);
		return min(c1,c2);
	...
	ll c1=cost(pl, middle);
	ll c2=cost(pl,middle+1);
	if(c1 < c2){
		return bin_search(pl,left,middle);
	}
	return bin_search(pl, middle+1, right);
}
...
public:
	long long minimizeCost(vector <int> positions, vector <int> lengths) {
		vector<pii> pl;
		REP(i, len(positions)){
			pl.pb(mp(positions[i], lengths[i]));
		}
		sort(pl.begin(), pl.end());
		return bin_search(pl, 0, 1000000000);
	}
};
