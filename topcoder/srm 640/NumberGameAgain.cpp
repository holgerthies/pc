...
long long number_of_children(long long n, long long k){
	long long deg_n = (long long)(log2(n));
	return pow(2,k-deg_n)-1;
}

bool is_child(long long n, long long k){
	if(n < k) return false;
	if(n == k) return true;
	return is_child(n/2, k);
}
	...
	long long solve(int k, vector<long long> table) {
		...
		for(int i=0; i<table.size();i++){
			...
			for(int j=i+1;j<table.size();j++){
				if(is_child(table[i], table[j])){
					insert=false;
					break;
				}
			}
			if(insert){
				table2.push_back(table[i]);
			}
		}
		long long ans = pow(2, k)-1;
		for(long long t : table2){
			ans -= number_of_children(t,k);
		}
		return ans-1;
	}
