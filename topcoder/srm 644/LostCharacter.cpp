...
bool cmp_funp(const string& a, const string& b, int pos=0){
	if((int)a.size() <= pos)
		return true;
	if((int)b.size() <= pos)
		return false;
	char ap = a[pos] == '?' ? 'z' : a[pos];
	char bp = b[pos] == '?' ? 'z' : b[pos];

	return ap < bp || (ap == bp && cmp_funp(a,b,pos+1));
}

bool cmp_fun(const string &a, const string &b){
	return cmp_funp(a,b);
}
...
public:
	vector <int> getmins(vector <string> str) {
			vector<string> sorted(str);
			sort(sorted.begin(), sorted.end(), cmp_fun);
			vector<int> ans;
			for(string s : str){
				string s2=replaceqm(s);
				int p = upper_bound(sorted.begin(), sorted.end(), s2, cmp_fun)-sorted.begin();
				//if(s2 != s) p--;
				ans.push_back(p);
			}
			return ans;
